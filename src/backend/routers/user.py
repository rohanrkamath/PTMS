...
from schema.user import UserUpdate, UserInDB, FirstNameUpdate, LastNameUpdate, PasswordUpdate, ProfilePicUpdate, PasswordUpdateResponse
...
user = APIRouter(
    prefix="/user",
    tags=["user"],
    dependencies=[Depends(require_role("user_router"))]
)
...
async def read_user(user_id: str, current_user: dict = Depends(require_role("user_router"))):
    try:
...
async def update_first_name(user_id: str, update_data: FirstNameUpdate, current_user: dict = Depends(require_role("user_router"))):
...
async def update_last_name(user_id: str, update_data: LastNameUpdate, current_user: dict = Depends(require_role("user_router"))):
...
async def update_password(user_id: str, update_data: PasswordUpdate, response: Response, current_user: dict = Depends(require_role("user_router"))):
...
async def update_profile_pic(user_id: str, update_data:ProfilePicUpdate, current_user: dict = Depends(require_role("user_router"))):
